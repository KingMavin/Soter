...
export class MockOnchainAdapter implements OnchainAdapter {
  ...
  async claimAidPackage(
    ...
    if (pkg.expiresAt <= nowSec) {
      pkg.status = 'Expired';
      throw new AppException(ERROR_CODES.BAD_REQUEST, 400, 'Aid package has expired');
    }
    ...
    if (
      ...
      throw new AppException(ERROR_CODES.BAD_REQUEST, 400, `Aid package is in status ${pkg.status}`);
    }
    ...
    if (amountToClaim <= BigInt(0)) {
      throw new AppException(ERROR_CODES.BAD_REQUEST, 400, 'Claim amount must be greater than zero');
    }

    if (amountToClaim > remaining) {
      throw new AppException(ERROR_CODES.BAD_REQUEST, 400, 'Claim amount exceeds remaining package balance',
      );
    }
